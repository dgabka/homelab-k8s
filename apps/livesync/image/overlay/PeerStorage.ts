...
export class PeerStorage extends Peer {
    ...
    private isExcluded(pathSrc: string): boolean {
        if (!this.config.exclude?.length) return false;
        const root = this.toStoragePath(this.toLocalPath("."));
        const absolute = resolve(pathSrc);
        const path = this.toPosixPath(absolute.startsWith(root) ? relative(root, absolute) : pathSrc);
        return this.config.exclude.some((pattern) => minimatch(path, pattern, { dot: true }));
    }
    ...
    async dispatch(pathSrc: string) {
        if (this.isExcluded(pathSrc)) return;
